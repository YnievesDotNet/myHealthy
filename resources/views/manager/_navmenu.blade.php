@if(($business = Session::get('selected.business')) && Auth::user()->isOwner($business))
    <li id="navHome"><a
                href="{{ route('manager.business.show', $business->id) }}">{!! Icon::home() !!} {{ $business->name }}</a>
    </li>
@endif

<li class="dropdown">
    <a href="#" class="dropdown-toggle" data-toggle="dropdown">{{ trans('app.nav.manager.business.menu') }} <b
                class="caret"></b></a>
